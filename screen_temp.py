...
config_loc = "YOUR CONFIG LOCATION"
...
class Temp_Pair:
    ...
    def __convert_to_hours(self, in_list):
        ...
        for in_value in in_list:
            ...
            out_times.append(in_t - in_t.replace(hour=0, minute=0, second=0, microsecond=0))
        return out_times

    def calc_redshift_temp(self, cur_time):
        if(not self.is_within_period(cur_time)):
            return POISON
        t_since_midnight = (cur_time - cur_time.replace(hour=0, minute=0, second=0, microsecond=0))
        t_since_midnight = self.__rollover_cur_t(t_since_midnight)
        time_diff_sec = (self.end_time - self.start_time).total_seconds()
        time_since_begin_sec = (t_since_midnight - self.start_time).total_seconds()
        temp_diff = self.end_temp - self.start_temp
        out_temp = ((time_since_begin_sec/time_diff_sec)*temp_diff) + self.start_temp
        return out_temp
    ...
    def is_within_period(self, cur_time):
        ...
        if (self.did_rollover):
            unrolled_end = self.end_time - timedelta(days=1)
            if ((t_since_midnight < self.start_time) and (t_since_midnight > unrolled_end)):
                return False
            else:
                return True
        else:
    ...
    for i in range(0,24):
        for j in range(0, 56, 5):
            ...
            if(n_pair.is_within_period(cur_t)):
                ...
                print("Time = {cur_t_str} | Temp = {cur_temp}".format(cur_t_str=cur_t_str, cur_temp=cur_temp))
...
def testing():
    ...
    for i in range(int(24*60/minute_diffs)):
        ...
        if (applicable_pair == None):
            print("Time = {cur_t} | No change".format(cur_t = str_time))
            continue
...
def smooth_transition(start, end):
    ...
    for i in range(int(abs(delta))):
        check_output("redshift -O  {cur_temp} -P".format(cur_temp=cur_temp), shell=True)
...
def get_pairs_list(config_fname):
    ...
    for user_pair in config.sections():
        ...
        pairs.append(Temp_Pair(start_time, end_time, start_temp, end_temp, user_pair))
    return pairs
...
def get_closest_after(pairs, cur_time):
    ...
    zero_time = timedelta()
    cur_time = cur_time - cur_time.replace(hour=0, minute=0, second=0, microsecond=0)
    for pair in pairs:
...
def get_closest_before(pairs, cur_time):
    ...
    cur_time = cur_time - cur_time.replace(hour=0, minute=0, second=0, microsecond=0)
    for pair in pairs:
        if (pair.did_rollover):
            end_time = pair.end_time - timedelta(days=1)
        else:
